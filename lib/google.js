...
export async function fetchDocDownload(id, format, kind = 'document') {
  if (!DOWNLOAD_TYPES[format]) throw new Error('Unsupported format');
  const res = await get(`https://docs.google.com/${kind === 'spreadsheet' ? 'spreadsheets' : 'document'}/d/${id}/export?format=${format}`, null);
  const type = res.headers.get('content-type') || '';
